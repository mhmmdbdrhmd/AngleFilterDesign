...
Rather than hard‐coding specific filenames, this workflow is designed to operate on **any new recording** you provide. The script auto-detects whether a file contains plain numbers or raw little-endian bit dumps and decodes them on the fly. As long as each recording provides the same logical columns (left/right sensor sums, a reference angle column, a speed column, and a timestamp), the same preprocessing, filtering, alignment, and evaluation steps will apply.
...
- **recordings/**: Directory where you place recording logs. The script accepts either the legacy semicolon-separated CSVs or the new raw-bit dumps (`index,0xID,byte0 … byte7`). In both cases the following logical columns must exist using the exact German names from the logger:
