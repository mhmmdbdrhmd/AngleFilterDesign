...
Rather than hard‐coding specific filenames, this workflow is designed to operate on **any new recording** you provide. The script auto-detects whether a file contains plain numbers or raw bit dumps (``index,0xID,byte0 … byte7``) and decodes them on the fly. Each payload must be exactly eight bytes; the loader infers little‑ or big‑endian encoding automatically. As long as each recording provides the same logical columns (left/right sensor sums, a reference angle column, a speed column, and a timestamp), the same preprocessing, filtering, alignment, and evaluation steps will apply.
...
- **recordings/**: Directory where you place recording logs. The script accepts either the legacy semicolon-separated CSVs or the new raw-bit dumps (`index,0xID,byte0 … byte7`, eight bytes per row). Endianness (little or big) is detected automatically. In both cases the following logical columns must exist using the exact German names from the logger:
...
1. **KF_inv** (Speed‐Aware Kalman Filter)
   ...
   - Starting from `y[0] = x[0]` and covariance `P[0] = 1`, the filter computes:
     - Predicted covariance `Pp = P[i-1] + Q` 
     - Kalman gain `K = Pp / (Pp + R)`
     - Updated state `y[i] = y[i-1] + K * (x[i] - y[i-1])`
     - Updated covariance `P[i] = (1 - K) * Pp`
