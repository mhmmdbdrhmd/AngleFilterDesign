...
def _read_raw_bitlog(path):
    """Read raw-bit recordings with automatic endianness detection.

    Each line is expected as ``index,0xID,byte0 byte1 ... byte7`` where the
    payload consists of **exactly eight bytes**. Sensor frames represent an
    IEEE-754 ``float64`` while the timestamp frame (``0x00A``) uses an unsigned
    64-bit integer. The byte order (little or big endian) is determined from the
    first valid non-timestamp frame. The function aggregates frames into rows
    ...
    # Determine byte order from first valid non-timestamp frame
    byteorder = "little"
    struct_fmt = "<d"
    for ln in lines:
        ...
        if len(data_bytes) != 8 or can_id == time_id:
            continue
        le = struct.unpack("<d", data_bytes)[0]
        be = struct.unpack(">d", data_bytes)[0]
        if 1e-9 < abs(be) < 1e6 and not (1e-9 < abs(le) < 1e6):
            byteorder, struct_fmt = "big", ">d"
        break
    ...
    for ln in lines:
        ...
        if can_id == time_id:
            if current:
                rows.append(current)
                current = {}
            current["ESP Time"] = int.from_bytes(data_bytes, byteorder)
        elif can_id in id_map:
            current[id_map[can_id]] = struct.unpack(struct_fmt, data_bytes)[0]
    if current:
        rows.append(current)
...
if __name__ == "__main__":
    ...
    # Update README results sections
    try:
        ...
        for readme in ["README.md", "DE_README.md"]:
            ...
            if table_start in lines and table_end in lines:
                ...
                label = "Leistungsübersicht" if readme.startswith("DE_") else "Performance Summary"
                table_md = tabulate(df_all_metrics, headers="keys", tablefmt="pipe", showindex=False)
                table_block = [f"<details><summary>{label}</summary>", ""] + table_md.splitlines() + ["</details>"]
